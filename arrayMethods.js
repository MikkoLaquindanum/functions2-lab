...
const totalPopulation = populations.reduce((previous, current) => previous + current, 0)
...
const totalOrders = orders.map(element => element.price * (1 + element.tax))
...
const findBob = purchases.filter(element => element.owner === "Bob")

const bobsTotal = findBob.reduce((p, c) => p + c.price, 0)
